...
- **Image paste support.** When the user pastes an image (e.g. a screenshot) into the terminal page, the blob is uploaded via `POST /api/clipboard-image?instance=<name>` to `/tmp/clipboard-<timestamp>-<rand>.<ext>`. For local instances this stays on the dashboard host. For pod instances the file is also `kubectl cp`'d into the pod's `/tmp/` so the agent running inside can read it, and the returned `path` is the pod-side path. A `[image: /tmp/...]` reference is injected into the active terminal session through `term.paste()` so the data lands in the right xterm.js session (not a phantom ttyd session). Listener is attached to both the outer page and the iframe's document so Cmd+V works whether the iframe or the outer chrome has focus.
